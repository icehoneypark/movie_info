...
❕ 백엔드를 담당해서 이번 프로젝트를 진행했지만 모르는 부분이 너무 많아 스스로의 한계를 많이 느꼈고, 그동안 개념을 제대로 잡지 못했구나라는 생각이 많이 들었습니다.

❕ 오탈자를 고치려고 쏟은 시간이 하루 남짓이 넘을 것 같은 기분이 들 정도로 간단한 실수 하나 때문에 프로젝트 전체가 지연되는 경우가 많았습니다. 조금 더 세세하게 코드를 작성하는 습관을 들여야 할 것 같습니다.

❕ 자꾸 html 템플릿에 파이썬 연산을 대입하는 시도를 했는데, 어떤 상황에 어떤 파트에서 코드를 짜는 것이 빠르고 보기 좋을지 한번 더 고민해보는 시간이 필요하다고 느꼈습니다.

❕ 짧다면 짧고 길다면 긴 시간을 달렸는데 생각 이상의 결과가 나온 것 같아 신기하고, 보완할 점은 시간을 들어 꼭 짚고 넘어가서 이후에 같은 실수를 반복하지 않게 할 생각입니다.
...
❕ 기본 기능 구현을 생각보다 빠르게 끝내 시간이 널널하다고 생각했는데, 추가적으로 tmdb api를 사용하고 이것저것 추가하면서 시간이 많이 부족해졌습니다. 하지만 api를 사용하며 다양한 동작을 추가할 수 있어서 재미있었습니다.

❕ 약 9일간 코드를 작성하고 오류를 수정하는 과정을 진행하면서 정말 많이 배운 것 같고 조금 힘든 감이 있었지만 결과물을 보니 속이 후련하고 뿌듯했습니다. 하지만 2학기 때 더 어렵고 긴 기간의 프로젝트를 3개 해야한다는 점과 이것을 vue로 해야한다는 점이 막막하고 준비를 단단히 해야할 것 같습니다.
