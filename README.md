...
✔ 기능 구현에 힘을더 쏟기 위해 기본 admin 페이지로 관리하도록 했다.
...
- 검색기능 추가
- Navbar 뒤로 홈페이지 이미지가 관통되어 보이는 현상 수정할 필요(가능하면 Navbar에 검색창 삽입)
